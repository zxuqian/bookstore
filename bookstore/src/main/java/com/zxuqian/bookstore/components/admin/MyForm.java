...
@Events({ EventConstants.PREPARE_FOR_RENDER, EventConstants.PREPARE,
		EventConstants.PREPARE_FOR_SUBMIT, EventConstants.VALIDATE,
		EventConstants.SUBMIT, EventConstants.FAILURE, EventConstants.SUCCESS,
		EventConstants.CANCELED })
@SupportsInformalParameters
public class MyForm implements ClientElement, FormValidationControl {
	 ...
	 * as an Ajax request that updates the indicated zone. Often a Form will
	 * update the same zone that contains it.
	 */
	...
	void beginRender(MarkupWriter writer) {
		...
		resources
				.triggerEvent(EventConstants.PREPARE_FOR_RENDER, context, null);
	...
	Object onAction(EventContext context) throws IOException {
		...
		try {
			...
			if (isFormCancelled()) {
				resources.triggerContextEvent(EventConstants.CANCELED, context,
						eventCallback);
				if (eventCallback.isAborted())
					return true;
			}
			...
			resources.triggerContextEvent(
					activeTracker.getHasErrors() ? EventConstants.FAILURE
